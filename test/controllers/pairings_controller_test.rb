...
class PairingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "opening a valid pairing link sets the pairing cookie and renders the install view linking the per-token manifest and the per-child icon (AE15)" do
    ...
    assert_select "link[rel=manifest][href=?]", "/p/#{@token}/manifest.webmanifest"
    assert_select "link[rel=apple-touch-icon][href=?]", "/p/#{@token}/icon-180.png"
  end

  test "opening the link with the launch marker sets the cookie and redirects to the root, rendering no install view" do
    get "/p/#{@token}?launch=1"
  ...
  test "the manifest's start URL carries both the token and the launch marker, and stays inside the root scope" do
    ...
    assert_equal "/p/#{@token}?launch=1", manifest["start_url"]
    ...
    assert_equal ["/p/#{@token}/icon-180.png", "180x180"], [small["src"], small["sizes"]]
    assert_equal ["/p/#{@token}/icon-512.png", "512x512"], [large["src"], large["sizes"]]
  ...
  test "a revoked or unknown token gets no icon, at either size, and no manifest" do
    ...
    get "/p/#{UNKNOWN_TOKEN}/icon-180.png"

    assert_response :not_found

    get "/p/#{UNKNOWN_TOKEN}/icon-512.png"
  ...
  test "a token containing a dot or a slash does not route" do
    get "/p/#{@token}.json"

    assert_response :not_found
    assert_select "[data-screen=lost-identity]", false

    get "/p/#{@token}/somewhere"
